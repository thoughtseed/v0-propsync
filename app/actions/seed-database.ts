...
import { seedSampleProperty } from "@/lib/seed-data/property-seed"
import { createClient } from "@/lib/supabase/server"

/**
 * Server action to seed the database with sample data
 * This can be called from a development script or the command line
 */
export async function seedDatabase() {
  try {
    // First, ensure the SQL function exists
    ...
    const { error: functionError } = await supabase.rpc("exec_sql", { sql: sqlFunction })
    if (functionError) {
      console.error("Error creating SQL function:", functionError)
      return { success: false, error: "Failed to create SQL function" }
    }

    // Now seed the sample property
    const result = await seedSampleProperty()
